...
def login():
    ...
    if current_user.is_authenticated:
        if current_user.can("GOV_ADMINISTRATOR"):
            return redirect(url_for('gov.gov_member'))
        elif current_user.can("AGENT_ADMINISTRATOR"):
            return redirect(url_for('agent.agent_member'))
        elif current_user.can("SYSTEM_ADMINISTRATOR"):
    ...
    if is_wechat_login:
        if openid is None:
            ...
            return redirect(url_for("admin_auth.login_bind", openid=openid))
        else:
            login_user(user)
            return redirect(url_for('main.index'))
    # 处理正常登录
    ...
    if form.validate_on_submit():
        ...
        elif not (user.can("GOV_ADMINISTRATOR") or user.can("AGENT_ADMINISTRATOR") or user.can("SYSTEM_ADMINISTRATOR")):
            flash("请使用管理员账号登录", "danger")
        else:  # 登录成功
            login_user(user, remember)
            if user.can("GOV_ADMINISTRATOR"):
                return redirect(url_for('gov.gov_member'))
            elif user.can("AGENT_ADMINISTRATOR"):
                return redirect(url_for('agent.agent_member'))
            elif user.can("SYSTEM_ADMINISTRATOR"):
                return redirect(url_for('system.admin_index'))
...
@admin_auth_bp.route('/get_verification_code', methods=['POST'])
...
@admin_auth_bp.route("/login_bind", methods=["GET", "POST"])
def login_bind():
    ...
    if form.validate_on_submit():
        ...
        flash('验证码错误', 'danger')
        return redirect_back()
    return render_template("admin/auth/bind_telephone.html", form=form)


@admin_auth_bp.route("/password_reset", methods=['GET', "POST"])
def password_reset():
    ...
    return render_template("admin/auth/password_reset.html", reset_form=reset_form)
